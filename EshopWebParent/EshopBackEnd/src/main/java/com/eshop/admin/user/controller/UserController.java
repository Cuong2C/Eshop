...
import com.eshop.admin.user.exporter.excelExporter;
import com.eshop.admin.user.exporter.userCsvExporter;
...
public class UserController {
	...
	public String saveUser(User user, RedirectAttributes redirect, @RequestParam("image") MultipartFile multipartFile) throws IOException {	
		...
		String uEmail = user.getEmail().split("@")[0];
		return "redirect:/users/page/1?sortField=email&sortDir=asc&keyword=" + uEmail;
	}
	...
	public String deleteUser(@PathVariable(name="id") Integer id, RedirectAttributes redirect, Model model ) {
		try {
	...
	public void exportCSV(HttpServletResponse reponse) throws IOException {
		...
		exporter.export(listUsers, reponse);
	}
